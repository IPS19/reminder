...
public class ReminderOwnershipAspect {
    ...
    public void checkOwnership(JoinPoint joinPoint, CheckUserOwnsReminder checkUserOwnsReminder) {
        ...
        if (reminderId == null) {
            throw new IllegalArgumentException("Не найден параметр с именем: " + paramName);
        }

        log.info("запрос в БД к reminder по reminderId");
        ...
        log.info("вытаскиваем авторизованного юзера");
        Long authUserId = AuthUser.get().id();

        if (reminder.isEmpty() || !Objects.equals(reminder.get().getUser().getId(), authUserId)) {
            throw new UnsupportedOperationException(
                    "У пользователя с id " + authUserId + " нет напоминания с id " + reminderId
            );
        }
    }
}
